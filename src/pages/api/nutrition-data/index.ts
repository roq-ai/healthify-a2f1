...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'POST':
      return createNutritionData();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getNutritionData() {
    const data = await prisma.nutrition_data
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'nutrition_data'));
  ...
  async function createNutritionData() {
    ...
    const data = await prisma.nutrition_data.create({
      data: body,
    });
    return res.status(200).json(data);
  }
